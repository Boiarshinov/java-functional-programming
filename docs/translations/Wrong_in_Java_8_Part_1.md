...
Это не так. 
Замыкания существуют в Java с самого начала. 
Но замыкания - зло. 
...
И да, и нет. 
Нет, потому что `a` и `b` - эффективно финальные, поэтому при выполнении функции они ведут себя как константы. 
...
Причиной тому является то, где вычисляются аргументы: все три вычисляются в разных местах. 
...
Каррирование - это способ обработки аргументов функции одного за другим, на каждом шаге получая новую функцию одного
аргумента. К примеру, если мы имеем следующею функцию
```
f(x, y, z) = x * y + z
``` 
...
Но мы можем автоматизировать это, написав метод, который будет принимать функцию двух аргументов и возвращать 
...
И на самом деле, это причина, почему `BiFunction` существует в Java 8: одно из частых применений функций 
- моделирование бинарных операторов.
...
Но разработчики, которые хотят писать функциональный код, столкнутся с тем, что, для того чтобы оставаться в 
...
В следующей статье мы поговорим о функциональных интерфейсах в Java 8 и о том, как обращаться с примитивными типами. 
